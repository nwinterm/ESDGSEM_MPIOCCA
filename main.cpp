...
if(MPI.rank==0){cout << "Initializing device...";}
...
if(MPI.rank==0){cout <<" ... device initialized.\n";}


if(MPI.rank==0){cout <<"declaring occa Kernels and Variables... ";}
...
if(MPI.rank==0){cout <<"Initializing Host Memory... ";}
...
if(MPI.rank==0){cout <<" ...Memory on Device allocated.\n";}
...
if (ArtificialViscosity){
    // add the additional viscose time derivatives to the inviscid time derivative
    UpdateQt                =   device.buildKernelFromSource("okl/ViscoseParts/UpdateQt.okl","UpdateQt",info);
    // collect edge information and boundary data, needed for BR1 computation of gradients
...
dfloat * mCheckpoints = (dfloat*) calloc(NumPlots,sizeof(dfloat));
    if (NumPlots>0){
        mCheckpoints[0]=0.0;
        if (NumPlots>1){
...
dfloat * tCheckpoints = (dfloat*) calloc(NumTimeChecks,sizeof(dfloat));
int timeCount=0;
tCheckpoints[0]=0.0;
...
if(MPI.rank==0){ cout << "Entering Time Loop!\n";}
//time run
...
//    cout << "globalLambdaMax: " << globalLambdaMax <<"\n";
...
if ( ArtificialViscosity==1){
    ...
    //(DFL/(N+1)^2)*deltaX^2/max(abs(epsilon));
    dt_v = DFL/(pow(ngl,2)) * pow(globalMinEleSize,2) / maxViscPara;

//    cout << "maxViscPara: " << maxViscPara <<"\n";
    dt = min(dt_i,dt_v);
}else{
    dt=dt_i;
    dt_v=0.0;
